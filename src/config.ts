...
export default function config(server: Server): void {
    ...
    server.configure(AutodiscoverConfigProviderFactory({
        imap: {
            host: process.env.IMAP_HOST || 'imap.example.com',
        },
        pop3: {
            host: process.env.POP3_HOST || 'pop3.example.com',
        },
        smtp: {
            host: process.env.SMTP_HOST || 'smtp.example.com',
        },
        activeSync: {
            url: process.env.ACTIVESYNC_URL || 'https://activesync.example.com/Microsoft-Server-ActiveSync'
        }
    }));
}
